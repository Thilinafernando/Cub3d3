...
/*   Updated: 2025/07/28 19:36:47 by tkurukul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_mydup(const char *s)
{
	...
	while (s[a] != '\0' && (!((s[a] >= 9 && s[a] <= 13) || s[a] == 32)))
		a++;
	str = (char *)ft_calloc((a + 1), sizeof(char));
	if (str == NULL)
		return (NULL);
	...
	while (s[i] != '\0' && (((s[i] >= 9 && s[i] <= 13) || s[i] == 32)))
		i++;
	if (s[i] != '\0')
		return (ft_printf(2, "Error: unwanted info.\n"), free(str), NULL);
	return (&str[0]);
}
...
int	rgb_convertion(t_info *info, char *str, int i, int j)
{
	...
	if (!rgb || rgb_next(&rgb))
		return (ft_printf(2, "Error: rgb value.\n"));
	tmp = ft_split(rgb, ',');
	...
	if (!ft_strncmp(str, "F", 1))
		return (rgb_fchecks(info, red, blue, green));
	else if (!ft_strncmp(str, "C", 1))
		return (rgb_cchecks(info, red, blue, green));
	return (0);
}
...
int	save_path(char *str, t_info *info, int i, int j)
{
	...
	close(fd);
	if (save_path_checks(str, &path, info))
		return (-1);
	return (0);
}
